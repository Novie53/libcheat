...
export const libcheat = {
	...
	addToggleable: (f, label, def = false) => {
	...
	addCheckbox: (label, variableName) => {
		if (libcheat.variables[variableName] !== undefined) { return "Error, a variable with that name already exists" }
		const div = document.getElementById(uidivid);
		const box = document.createElement('input');
		const text = document.createElement('span');
		text.style.color = '#66FF33';
		text.innerText = label;
		box.setAttribute("type", "checkbox");
		libcheat.variables[variableName] = false;
		div.appendChild(box);
		div.appendChild(text);
		div.appendChild(document.createElement('br'));
		
		box.addEventListener("click", function() {
			libcheat.variables[variableName] = box.checked;
		});
	},
	addTextbox: (label, variableName, textBoxWidth = "20px") => {
		if (libcheat.variables[variableName] !== undefined) { return "Error, a variable with that name already exists" }
		const div = document.getElementById("cheat_ui_div");
		const input = document.createElement('input');
		const text = document.createElement('span');
		text.style.color = '#66FF33';
		text.innerText = label;
		input.style.width = width;
		libcheat.variables[variableName] = "";
		div.appendChild(text);
		div.appendChild(input);
		div.appendChild(document.createElement('br'));
		
		input.addEventListener("change", function(e) {
			libcheat.variables[variableName] = e.target.value;
		});
	},
	addCheckBoxAndTextBox: (label, checkBoxVaribleName, textBoxVariableName, textBoxWidth = "20px") => {
		if (libcheat.variables[checkBoxVaribleName] !== undefined || 
			libcheat.variables[textBoxVariableName] !== undefined) { return "Error, a variable with that name already exists" }
		...
		text.innerText = label;
		box.setAttribute("type", "checkbox");
		libcheat.variables[checkBoxVaribleName] = false;
		libcheat.variables[textBoxVariableName] = "";
		input.style.width = textBoxWidth;
		div.appendChild(box);
		div.appendChild(text);
		div.appendChild(input);
		div.appendChild(document.createElement('br'));
		...
		box.addEventListener("click", function() {
			libcheat.variables[checkBoxVaribleName] = box.checked;
		});
		input.addEventListener("change", function(e) {
			libcheat.variables[textBoxVariableName] = e.target.value;
		});
	}
};
